...
export class MMPaginatorComponent implements OnInit {
  // prevText: text inside button for going to previous page
  @Input() prevText: string = 'Prev';
  // nextText: text inside a button for going to next page
  ...
  checkForErrors() {
    ...
    if (this.pagesToShowAroundActive < -1 || this.pagesToShowAroundActive > this.dataLength) {
  ...
  // warning messages
  pagesToShowAroundActiveError = 'PAGINATOR WARNING:<br>Incorrect number entered for displaying pages before/after the active one!<br>Only -1 and higher values are accepted, and value cannot be higher than your *data.length* value<br>Check the *pagesToShowAroundActive* value.';
  entriesToShowError = 'PAGINATOR WARNING:<br>Incorrect number entered for amount of entries per page!<br>Minimum of 1 is required, and the maximum cannot exceed the *data.length* value<br>Check the *entriesToShow* value.';
