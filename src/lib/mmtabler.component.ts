...
export class MMTablerComponent implements OnInit {
  ...
  checkForWarnings(conf: any) {
    ...
    if (!conf || Object.keys(conf).length === 0) this.warningMessages('configuration-missing');
    // if no entriesToshow has been passed to table
    else if (!this.entriesToShow) this.warningMessages('entriestoshow-missing');
    // if no activePage has been passed to table
    else if (!this.activePage) this.warningMessages('activepage-missing');
    // if configuration is set to receive emitted data from MMTabler, but no receiving method has been passed to MMTabler
    else if (!this.sendChangesToParent.observers.length && conf.sendChanges) this.warningMessages('update-missing');
    // If no table data has been passed
    else if (!this.data) this.warningMessages('data-missing');
    // if customHeaders is missing, but customHeadersOrder is provided
    else if (conf.customHeadersOrder && !conf.customHeaders) this.warningMessages('headers-order-missing');
    // if customHeaders and customHeadersOrder don't have the same values
    else if (conf.customHeadersOrder && conf.customHeaders) {
      // check if each customHeadersOrder item is present in customHeaders
      let allCustomHeaders = Object.values(conf.customHeaders);
      conf.customHeadersOrder.forEach((header) => {
        if (!allCustomHeaders.includes(header)) this.warningMessages('headers-order-issue');
      });
    }
    // if configuration is set to use custom headers, but no custom headers have been passed
    else if (conf.displayHeadersFromCustomHeaders && !conf.customHeaders) this.warningMessages('header-dataHeaders-missing');
    // if configuration is set to use both data headers and custom headers
    else if (conf.displayHeadersFromData && conf.displayHeadersFromCustomHeaders) this.warningMessages('header-dataHeaders-double');
    // if configuration is set to allow options, but no option has been set
    else if (conf.allowOptions && !conf.allowEdit && !conf.allowDelete && !conf.allowAdd) this.warningMessages('allowances-missing');
    // if configuration is set to use all 3 possible stylings for headers
    else if (conf.headersToUppercase && conf.headersToCapitalized && conf.headersToLowercase) this.warningMessages('headers-collision-1');
    // if configuration is set to use any 2 of the possible stylings for headers
    else if (
      (conf.headersToUppercase && conf.headersToCapitalized && !conf.headersToLowercase) ||
      (conf.headersToUppercase && !conf.headersToCapitalized && conf.headersToLowercase) ||
      (!conf.headersToUppercase && conf.headersToCapitalized && conf.headersToLowercase)
    ...
    else if (conf.displayHeadersFromCustomHeaders && conf.initialOrderBy && !Object.values(conf.customHeaders).includes(conf.initialOrderBy)) this.warningMessages('headers-mismatch');
  }
  ...
  setHeadersFromCustom(conf: any) {
    ...
    this.headers = Object.keys(this.dataFinal[0]);
    // removes unneeded headers, based on configuration.customHeadersOrder
    if (conf.customHeadersOrder) {
      this.dataFinal.forEach((entry) => {
        for (const property in entry) {
          if (!conf.customHeadersOrder.includes(property)) {
  ...
  warningMessages(type) {
    let msg = '';
    switch (type) {
      case 'headers-order-issue':
        msg = `<b>HEADERS MISCONFIGURATION</b><br><br>You've set customHeadersOrder and customHeaders in cofiguration, but values don't match. Check the values for typos.`;
        break;
      case 'headers-order-missing':
        msg = `<b>HEADERS MISCONFIGURATION</b><br><br>You've set customHeadersOrder in configuration, but didn't provide customHeaders that customHeadersOrder will use. Set both options.`;
        break;
